/**
 * Better YouTube Firefox Integration Test - ext-9
 * Tests Shorts hiding functionality on Firefox
 * 
...
const USER_DATA_DIR = path.join(EXTENSION_ROOT, ".tmp-playwright-profile-firefox-ext-9");
...
const run = async () => {
  ...
  try {
    ...
    console.log("✅ Firefox ext-9 test passed");
  } finally {
    await context.close();
  }
};
...
run().catch((error) => {
  console.error("❌ Firefox ext-9 test failed:", error.message || error);
  process.exitCode = 1;
});
